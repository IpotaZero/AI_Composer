...
def Learn():
    def learning(phrases):
        ...
        for h, phrase in enumerate(phrases):
            for i in range(len(phrase)):
                ...
                for j in range(max_size):
                    ...
                    connection = next(
                        (c for c in connections if c["a"] == subphrase), None
                    )
    ...
    def generate_melody(key_note):
        ...
        while length < max_length:
            ...
            if connection is not None and len(connection["b"]) > 0:
                # 次の音が存在するなら
                total = sum(connection["c"])
                probabilities = [count / total for count in connection["c"]]
                selected_index = random.choices(
                    range(len(connection["b"])), weights=probabilities
                )[0]
...
def Introduce():
    ...
    text.insert(tkinter.END, "めんどかったのでbpm=120で固定\n")
    text["state"] = "disabled"


def load_module():
    return Learn, Generate, Introduce

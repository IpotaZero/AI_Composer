import glob
import sys
from window import menubar, com_select, get_file_extension, addlog
import threading
...
def load_module():
    ...
    for ppath in glob.glob(ai_path + "/*"):
        if get_file_extension(ppath) == "pyd" or os.path.basename(ppath) == "ComIII.py":
